app.controller('QuizStudentCtrl', [
  '$scope',
  '$rootScope',
  'QuizManager',
  'DataManager',
    ...
    DataManager.getData(type, function (data) {
      if (data && (data.node != QuizManager.node || !$rootScope.isEnd)) {
        ...
        QuizManager.load().then(function (quiz) {
          var index = $rootScope.index, select = 0, n = quiz.length;
...
app.controller('QuizTeacherCtrl', [
  '$scope',
  'QuizManager',
  'DataManager',
  function ($scope, QuizManager, DataManager) {
    ...
    if (angular.isDefined(QuizManager.node)) {
      DataManager.setData(type, { node: QuizManager.node });
    ...
    LoginManager.getUser().then(function (user) {
      ...
      $scope.saveDraw = function () {
        $scope.showDialog().then(function (name) {
          ...
          var data = cs.toDataURL({ format: type.split('/') });
          var obj = {};
          obj.type = type;
...
app.controller('HomeTeacherCtrl', [
  '$scope',
  '$modal',
  '$rootScope',
  'Room',
...
app.controller('HomeStudentCtrl', [
  '$scope',
  '$rootScope',
  '$modal',
  'Room',
