...
app.directive('selector', [
  ...
  function ($rootScope, $modal, DrawFactory) {
    return {
      restrict: 'E',
      template: '<div class="width-100 fit-height" ng-click="dialog()"><div id="attr">' + '<div id="color" class="pull-left"></div>' + '<div class="pull-left white margin-10">' + '<div id ="size" class="icon-circle align-middle"></div>' + '<span class="bigger-140 align-middle">{{selector.size}}<span>' + '</div></div></div>',
      link: function (scope, iElement, iAttrs) {
        ...
        scope.$watch('selector.size', function () {
          var scale = scope.selector.size / 50 * 100;
